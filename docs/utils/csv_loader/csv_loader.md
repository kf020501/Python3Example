...
from src.utils.csv_loader import csv_to_matrix, matrix_to_list_dict

def load_items_from_csv() -> list[dict[str, object]]:

    # CSVパスを指定
    project_root = Path(__file__).resolve().parent.parent
    csv_rel_path = "resources/items.csv"  # PJルートからの相対パス
    csv_path = project_root / csv_rel_path
    
    # csv_to_matrixの例
    headers, rows = csv_to_matrix(str(csv_path))
    print("headers:", headers)
    print("rows:", rows)

    # matrix_to_list_dictの例
    list_dict = matrix_to_list_dict(headers, rows)
    print("matrix_to_list_dict:", list_dict)
```
...
```python
# csv_to_matrixの例
headers: ['id', 'value', 'flag', 'comment']
rows: [[1, 123, True, None], [2, '123', False, 'hello,world']]

# matrix_to_list_dictの例
matrix_to_list_dict: [{'id': 1, 'value': 123, 'flag': True, 'comment': None}, {'id': 2, 'value': '123', 'flag': False, 'comment': 'hello,world'}]
```
